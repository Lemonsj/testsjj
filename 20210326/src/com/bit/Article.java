...
public class Article {
    ...
    public Article(int id, User author, String title, image image1, String body) {
        this.id = id;
        this.author = author;
        this.title = title;
        this.image1 = image1;
    ...
    public static Article publish(User author, String title, image image1, String body) throws ServletException {
        try (Connection con = DB.getConnection()) {
            String sql = "INSERT INTO articles (author_id, image_id, title, body) VALUES (?, ?, ?, ?)";
            try (PreparedStatement stmt = con.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
                stmt.setInt(1, author.id);
                stmt.setInt(2, image1.id);
                ...
                try (ResultSet rs = stmt.getGeneratedKeys()) {
                    if (rs.next()) {
                        int id = rs.getInt(1);
                        return new Article(id, author, title, image1, body);
                    } else {
                        return null;
                    }
                }
